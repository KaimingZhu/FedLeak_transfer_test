"""
@Encoding:      UTF-8
@File:          adapted_generator.py

@Introduction:  Our adapted generator in result, transferred from `./resize_generator.py`. This Generator is used to conduct 
                all the experiments in our paper.
@Author:        Kaiming Zhu
@Date:          2026/1/4 20:38:21
@Notes:         Comparing to `./resize_generator.py`, we add a optional feature for FedLeak: reporting the  best checkpoint. 
                Specifically, the result returned by `FedLeak` could be derived from the `reconstructed_imgs` that achieve 
                the minimum loss (i.e., `total_loss`). 
                
                This behaviour could be triggered with the param `need_best_checkpoint_reporting`, and disabled by default.
"""
...
def make_resizing_func(generated_res: int, expected_res: int) -> Callable[[torch.Tensor], torch.Tensor]:
    ...
    if generated_res == expected_res:
        ...
        return lambda x: F.interpolate(x, size=(expected_res, expected_res), mode="bilinear", align_corners=False) 
...
# 🌟 New
def FedLeak(client_grads, original_label, model, grad_diff_loss, img_res=128, need_upscaling=True, batch_size=16, num_iters=10000, plot_interval=1000, need_best_checkpoint_reporting=False, device=None):
    ...
    # 🌟 New
    if need_best_checkpoint_reporting:
        best_state_dict = copy_state_dict(netG.state_dict())
